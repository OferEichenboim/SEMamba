...
def train(rank, args, cfg):
    ...
    cfg_SEMamba = cfg.copy()
    cfg_SEMamba['model_cfg'] = cfg_SEMamba['pretrained_model_cfg']
    pretrained_model = SEMamba(cfg_SEMamba).to(device)
    state_dict = torch.load(args.checkpoint_file, map_location=device)
    pretrained_model.load_state_dict(state_dict['generator'])
    pretrained_model.eval()


    #generator = SEMamba(cfg).to(device)
    generator_basic = BasicMambaGenerator(cfg)
    ...
    if rank == 0:
        ...
        print(f"writer path: {os.path.join(args.exp_path, 'logs')}")
    ...
    for epoch in range(max(0, last_epoch), cfg['training_cfg']['training_epochs']):
        ...
        for i, batch in enumerate(train_loader):
            ...
            mag_g_pretrained, pha_g_pretrained, com_g_pretrained = pretrained_model(noisy_mag, noisy_pha)
            # audio_g_pretrained = mag_phase_istft(mag_g_pretrained, pha_g_pretrained, n_fft, hop_size, win_size, compress_factor)
            # audio_g = audio_g / norm_factor
            # new model
            mag_g = generator(noisy_audio, noisy_mag)
        
            #inverse log1p(x)
            mag_g = torch.exp(mag_g) - 1
            ...
            # try:
            #     audio_list_r = list(clean_audio.cpu().detach().numpy())
            #     audio_list_g = list(audio_g.cpu().detach().numpy())
            # except RuntimeError as e:
            #     print(f"Error converting to NumPy: {e}")
            #     print(f"clean_audio device: {clean_audio.device}")
            #     print(f"audio_g device: {audio_g.device}")
            ...
            # ------------------------------------------------------- #
            optim_g.zero_grad()
            # Only MSE loss on MAG because our trained model fixes magnitude only.
            loss_gen_all = F.mse_loss(mag_g_pretrained, mag_g) * 1e2
            loss_gen_all.backward()
            torch.nn.utils.clip_grad_norm_(generator.parameters(), max_norm=cfg["training_cfg"]["weight_clipping"]) 
            optim_g.step()
            # ------------------------------------------------------- #
            if rank == 0:
                # STDOUT logging
                if steps % cfg['env_setting']['stdout_interval'] == 0:
                    with torch.no_grad():
                        ...
                        print(
                            'Steps : {:d}, Gen Loss: {:4.3f}, '
                            'Mag Loss: {:4.3f}, Time Loss: {:4.3f} s/b : {:4.3f}'.format(
                                steps, loss_gen_all, mag_error,  time_error, time.time() - start_b
                            )
                        )
                ...
                if steps % cfg['env_setting']['validation_interval'] == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            mag_g = generator(noisy_audio.to(device), noisy_mag.to(device))
                            mag_g = torch.exp(mag_g) - 1
                    ...
                    print(f"valid: PESQ {val_pesq_score}, Mag_loss {val_mag_err}, Best_PESQ: {best_pesq} at step {best_pesq_step}")
            steps += 1
...
def main():
    ...
    parser.add_argument('--exp_name', default='SEMamba_fusion')
    parser.add_argument('--config', default='recipes/SEMamba_fusion/SEMamba_fusion.yaml')
    parser.add_argument('--checkpoint_file', default='recipes/SEMamba_fusion/SEMamba_fusion.yaml')
